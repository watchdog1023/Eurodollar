...
The RPM spec file provided here is for Eurodollar-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://eurodollar.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Eurodollar git repository](https://github.com/eurodollar/eurodollar) but are not part of
...
Currently only `Source10` is used, for the example `eurodollar.conf` file.
...
in the eurodollar git repository. At present only an SVG version of the Eurodollar
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the eurodollar
...
needed when 0.12.1 is released, a proper fix is already in the Eurodollar git
master branch.
...
idea to build Eurodollar Core against a static version of that library compiled
...
The `eurodollar.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `eurodollar-server` package. At this point, most RPM based
...
The `eurodollar-server` package should have SELinux support. How to properly do
...
Eurodollar-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/eurodollar/). He has been
packaging Eurodollar for Fedora at least since 2012.
...
4. I renamed the `eurodollar` package that contains the Qt GUI to `eurodollar-core` as
that appears to be how the general population refers to it, in contrast to
`eurodollar-xt` or `eurodollar-classic`. I wanted to make sure the general population
