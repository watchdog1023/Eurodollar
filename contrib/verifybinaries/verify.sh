...
###   eurodollarcore.org and eurodollar.org and compares them.
...
WORKINGDIR="/tmp/eurodollar_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://eurodollarcore.org"
HOST2="https://eurodollar.org"
BASEDIR="/bin/"
VERSIONPREFIX="eurodollar-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'eurodollar-',
...
if [ $? -ne 0 ]; then
   echo "eurodollar.org failed to provide signature file, but eurodollarcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "eurodollar.org and eurodollarcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Eurodollar Core binary release signing key installed?"
   fi
