#!/bin/bash
# Copyright (c) 2013 The Eurodollar Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Eurodollar database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Eurodollar datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Eurodollar datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Eurodollar 0.7 datadir."
    ;;
  3)
    echo "Detected Eurodollar pre-0.8 datadir."
    ;;
  4)
    echo "Detected Eurodollar 0.8 datadir."
    ;;
esac
