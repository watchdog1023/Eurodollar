Eurodollar Core version 0.11.0 is now available from:

  <https://eurodollar.org/bin/eurodollar-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer EUD/kB fee to be
...
For example, add the following to `eurodollar.conf`:
...
related to the blockchain in the eurodollar system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Eurodollar Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Eurodollar Core will 
...
For an example script see [eurodollar-submittx](https://github.com/laanwj/eurodollar-submittx).
...
- #5334 `cf87536` libeurodollarconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new eurodollar-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only EurodollarAmountField
- #5753 `0f44672` Add eurodollar logo to about screen
...
- #5809 `46bfbe7` Add eurodollar-cli man page
...
- #5849 `41259ca` contrib/init/eurodollard.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Eurodollar-Qt.app to Eurodollar-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for eurodollar-tx)
